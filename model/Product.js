...
const Product = {
    getById : async function(productId){
        return await productsColl.find({_id: new ObjectID(productId)});
    },
    getByCategory : async function(categoryId) {
        products = await productsColl.find({categoryId: new ObjectID(categoryId)}).toArray();
        return products;
    },
    ...
    getAll : async function() {
        ...
        for (let data of categoriesId){
            products = await productsColl.find({categoryId: new ObjectID(data)}).toArray();
        }
        return products;
        /*return [
